...
Clear-Host
# Konsolenfarben auf Schwarz/Weiß setzen
$Host.UI.RawUI.BackgroundColor = "Black"
$Host.UI.RawUI.ForegroundColor = "White"
Clear-Host # Erneut löschen, damit die Hintergrundfarbe sofort angewendet wird
...
# --- NEU: Graceful Exit Funktion ---
function Graceful-Exit {
    Clear-Host
    Write-Header "RESTART REQUIRED"
    Write-Host ""
    "A manual reboot is required for all changes to take effect!".ToCharArray() | ForEach-Object { Write-Host -NoNewline $_ -ForegroundColor Cyan; Start-Sleep -Milliseconds 10 }; Write-Host ""
    Write-Host ""
    "This window will close automatically in 5 seconds...".ToCharArray() | ForEach-Object { Write-Host -NoNewline $_ -ForegroundColor Gray; Start-Sleep -Milliseconds 10 }; Write-Host ""
    Start-Sleep -Seconds 5
    exit
}

# --- NEU: SCRIPT START COUNTDOWN ---
Clear-Host
...
Write-Hacker -Text "- Disabling unnecessary virtual/WAN network adapters" # Added new feature
Write-Host ""
...
Write-Host "`n[1/18] Configuring NIC interrupt settings..." -ForegroundColor Yellow
...
Write-Host "`n[2/18] Applying advanced NETSH commands..." -ForegroundColor Yellow
$netshBaseCommands = @(
...
Write-Host "`n[3/18] Configuring NDIS parameters..." -ForegroundColor Yellow
$ndisProperties = @{
...
Write-Host "`n[4/18] Configuring NetBT settings..." -ForegroundColor Yellow
...
Write-Host "`n[5/18] Advanced Interface optimization..." -ForegroundColor Yellow
...
Write-Host "`n[6/18] Optimizing TCP/IP global parameters..." -ForegroundColor Yellow
$tcpipProperties = @{
    "EnableConnectionRateLimiting" = 0; # HINZUGEFÜGT
    "EnablePMTUDiscovery" = 1; "TcpAckFrequency" = 1; "TcpDelAckTicks" = 0; "TCPNoDelay" = 1;
    "MaxUserPort" = 65534; "DefaultTTL" = 64; "TcpMaxDataRetransmissions" = 5; "TcpTimedWaitDelay" = 30;
    "DefaultReceiveWindow" = 65536; "DefaultSendWindow" = 65536; "DisableTaskOffload" = 0;
    "EnableICMPRedirect" = 1; "DeadGWDetectDefault" = 1; "DontAddDefaultGatewayDefault" = 0;
    "InitialCongestionWindow" = 10; "UseDomainNameDevolution" = 1; "EnableIPAutoConfigurationLimits" = 0;
    "UdpNoDelay" = 1; "UDPMaxSockets" = 65535; "UDPReceiveBufferSize" = 65536; "UDPSendBufferSize" = 65536
}
...
Write-Host "`n[7/18] Configuring Winsock parameters..." -ForegroundColor Yellow
...
Write-Host "`n[8/18] Configuring Service Provider Priority..." -ForegroundColor Yellow
$serviceProviderProperties = @{
    "Class" = 8; "DnsPriority" = 6; "HostsPriority" = 5;
    "LocalPriority" = 4; "NetbtPriority" = 7
}
Set-RegistryProperties "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\ServiceProvider" $serviceProviderProperties
Write-ColorText "       - ✓ Service Provider Priorities for name resolution optimized" "Green"

Write-Host "`n[9/18] Optimizing AFD parameters..." -ForegroundColor Yellow
$afdProperties = @{
    # Allgemeine AFD-Optimierungen
    "FastSendDatagramThreshold" = 1500;
    "FastCopyReceiveThreshold"  = 1500;
    "DynamicSendBufferDisable"  = 1;
    "MinimumDynamicBacklog"     = 10;
    "MaximumDynamicBacklog"     = 64;
    "AfdDoNotHoldNICBuffers"    = 1;

    # Verbesserte UDP-Parameter für Gaming & Low Latency
    "UDPMaxSockets"             = 65535;  # Maximalwert, verhindert Knappheit an Sockets
    "UDPReceiveBufferSize"      = 32768;  # Erhöhter Puffer (32 KB) zur Vermeidung von Packet Loss
    "UDPSendBufferSize"         = 32768   # Erhöhter Puffer (32 KB) für ausgehende Pakete
}
Set-RegistryProperties "HKLM:\SYSTEM\CurrentControlSet\Services\AFD\Parameters" $afdProperties
Write-ColorText "       - ✓ AFD parameters optimized (with improved UDP buffers)" "Green"

Write-Host "`n[10/18] Configuring system priority control..." -ForegroundColor Yellow
...
Write-Host "`n[11/18] Disabling general power management features..." -ForegroundColor Yellow
...
Write-Host "`n[12/18] Applying Realtek-specific optimizations (including advanced CLKREQ fix)..." -ForegroundColor Yellow
...
if ($realtekNic) {
    ...
    # --- ADVANCED PROPERTIES VIA POWERSHELL CMDLETS (CLKREQ & Power Management) ---
    Try {
    ...
    } Catch { Write-ColorText "       - Error disabling 'Allow PC to turn off device to save power': $($_.Exception.Message)" "Red" }
    ...
    Try {
        ...
        } else { Write-ColorText "       - [✓] 'Energy Efficient Ethernet' already disabled or not applicable." "Green" }
    } Catch { Write-ColorText "       - Error disabling 'Energy Efficient Ethernet': $($_.Exception.Message)" "Red" }
    ...
    foreach ($prop in $clkreqAdvancedProperties) {
        Try {
            Set-NetAdapterAdvancedProperty -Name $realtekNic.Name -RegistryKeyword $prop.RegistryKeyword -RegistryValue $prop.Value -ErrorAction Stop
            Write-ColorText "       - [✓] '$($prop.RegistryKeyword)' successfully set via PowerShell." "Green"
        } Catch {
            Write-ColorText "       - [i] Property '$($prop.RegistryKeyword)' not available via PowerShell (will be set directly in registry)." "Yellow"
        }
    }

    # --- REGISTRY SETTINGS VIA DYNAMIC PATH ---
    $nicRegPathBase = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e972-e325-11ce-bfc1-08002be10318}"
    $adapterInstanceKey = (Get-ItemProperty -Path "${nicRegPathBase}\*" -ErrorAction SilentlyContinue | Where-Object { $_.NetCfgInstanceId -eq $realtekNic.InstanceID }).PSPath

    if ($adapterInstanceKey -and (Test-Path $adapterInstanceKey)) {
        Write-ColorText "       - ✓ Dynamically found correct registry path: $adapterInstanceKey" "Green"
        
        # --- COMBINED REALTEK SETTINGS (OFFLOADS, POWER, ETC.) ---
        $allRealtekSettings = @{
            "*SpeedDuplex" = "6"; "*FlowControl" = "0"; "*InterruptModeration" = "0"; "*IPChecksumOffloadIPv4" = "0";
            "*TCPChecksumOffloadIPv4" = "0"; "*UDPChecksumOffloadIPv4" = "0"; "*TCPChecksumOffloadIPv6" = "0"; "*UDPChecksumOffloadIPv6" = "0";
            "*LsoV1IPv4" = "0"; "*LsoV2IPv4" = "0"; "*LsoV2IPv6" = "0"; "*JumboPacket" = "1514"; "*ReceiveBuffers" = "1024"; "*TransmitBuffers" = "512";
            "*RSS" = "0"; "*NumRssQueues" = "1"; "*DeviceSleepOnDisconnect" = "0"; "*NicAutoPowerSaver" = "0"; "EnablePowerManagement" = "0";
            "*EEE" = "0"; "ULPMode" = "0"; "PowerSavingMode" = "0"; "*WakeOnPattern" = "0"; "*WakeOnMagicPacket" = "0"; "*PriorityVlanTag" = "0";
            "S5WakeOnLan" = "0"; "WakeFromS5" = "0"; "*PMARPOffload" = "0"; "*PMNSOffload" = "0"; "*EncapsulatedPacketTaskOffloadNvgre" = "0";
            "*EncapsulatedPacketTaskOffloadVxlan" = "0"; "*IPsecOffloadV1IPv4" = "0"; "*IPsecOffloadV2IPv4" = "0"; "*IPsecOffloadV2" = "0";
            "*UsoIPv4" = "0"; "*UsoIPv6" = "0"; "EnableUdpTxScaling" = 0; "EnableConnectedPowerGating" = 0; "DisableDelayedPowerUp" = 1; "EnableUPS" = 0;
            "DisablePwrSavingOnRST" = 1; "EnableExtraPowerSaving" = 0; "EnableD3Cold" = 0; "EnableD3ColdInterface" = 0;
            "EnableOOBSelectiveSuspend" = 0; "ALDPS" = 0; "EnablePowerCut" = 0; "*PacketDirect" = 1; "ASPM" = 0; "CLKREQ" = 0;
            "HwOption" = 0x00C00000; "HwOptionV2" = 0x00000004; "HwOptionV3" = 0x00040000; "RM2779240" = 5; "PcieClkReqUsePort" = 0;
            "*ClkReqEnable" = 0; "*ClkReqSupported" = 0; "EnableClkReq" = 0; "*MSIXClkReq" = 0;
        }

        Set-RegistryProperties $adapterInstanceKey $allRealtekSettings
        Write-ColorText "       - ✓ All Realtek optimizations applied to the correct adapter path." "Green"

    } else {
        Write-ColorText "       - Could not find specific registry path for Realtek network adapter. Some tweaks skipped." "Yellow"
    }
} else {
...
Write-Host "`n[13/18] Configuring hardware offloading..." -ForegroundColor Yellow
$adaptersToConfig = Get-NetAdapter -ErrorAction SilentlyContinue
if ($adaptersToConfig) {
    netsh int tcp set global chimney=disabled | Out-Null
    Set-NetOffloadGlobalSetting -PacketCoalescingFilter Disabled -ErrorAction SilentlyContinue | Out-Null # HINZUGEFÜGT
    ...
    Write-ColorText "       - ✓ Hardware offloading disabled (including Packet Coalescing)" "Green"
} else {
    Write-ColorText "       - WARNING: Network adapters could not be queried. Skipping Offloading." "Yellow"
}

Write-Host "`n[14/18] Finalizing TCP/IP stack optimization..." -ForegroundColor Yellow
...
netsh int tcp set global autotuninglevel=normal | Out-Null; netsh int tcp set global ecncapability=enabled | Out-Null; netsh int tcp set global timestamps=disabled | Out-Null; netsh int tcp set global initialRto=300 | Out-Null; netsh int tcp set global rss=disabled | Out-Null # GEÄNDERT
Write-ColorText "       - ✓ Final TCP settings applied" "Green"

Write-Host "`n[15/18] Creating persistent CLKREQ task (comprehensive fix)..." -ForegroundColor Yellow
...
Write-Host "`n[16/18] Disabling unnecessary virtual/WAN network adapters..." -ForegroundColor Yellow
$adaptersToDisable = @(
...
foreach ($adapterName in $adaptersToDisable) {
    Try {
        $pnpDevice = Get-PnpDevice -Class Net -FriendlyName $adapterName -ErrorAction SilentlyContinue
        if ($pnpDevice) {
            if ($pnpDevice.Status -ne "OK" -or $pnpDevice.Problem -ne 0) { # Check if it's already disabled or has a problem
                Write-ColorText "       - [✓] '$adapterName' already disabled or not active (Status: $($pnpDevice.Status), Problem: $($pnpDevice.Problem))." "Green"
            } else {
                Disable-PnpDevice -InstanceId $pnpDevice.InstanceId -Confirm:$false -ErrorAction Stop | Out-Null
                Write-ColorText "       - [✓] Disabled '$adapterName'." "Green"
            }
        } else {
            Write-ColorText "       - [✓] '$adapterName' not found on this system." "Green"
        }
    } Catch {
        Write-ColorText "       - [✗] ERROR: Could not disable '$adapterName': $($_.Exception.Message)" "Red"
    }
}
Write-ColorText "       - ✓ Unnecessary virtual/WAN adapters configuration complete." "Green"

# HINZUGEFÜGT: Neuer Schritt für QoS mit Kompatibilitätsprüfung
Write-Host "`n[17/18] Creating Fortnite QoS Policy for traffic prioritization..." -ForegroundColor Yellow
$qosPolicyCreated = $false
if (Get-Command -Name New-NetQosPolicy -ErrorAction SilentlyContinue) {
    $fortniteExe = "FortniteClient-Win64-Shipping.exe"
    $qoSProfileName = "Fortnite High Priority"
    # Vorhandene Richtlinie zuerst entfernen
    Get-NetQosPolicy -Name $qoSProfileName -ErrorAction SilentlyContinue | Remove-NetQosPolicy -Confirm:$false
    Try {
        New-NetQosPolicy -Name $qoSProfileName -AppPathNameMatchCondition $fortniteExe -IPProtocolMatchCondition Both -DSCPAction 46 -ErrorAction Stop
        Write-ColorText "       - ✓ Successfully created QoS policy '$qoSProfileName' for '$fortniteExe'." "Green"
        $qosPolicyCreated = $true
    } Catch {
        Write-ColorText "       - [✗] ERROR: Could not create QoS policy." "Red"
    }
} else {
    Write-ColorText "       - [i] QoS cmdlets not available on this Windows edition. Skipping feature." "Yellow"
}

Write-Host "`n[18/18] Finalizing with network stack reset..." -ForegroundColor Yellow
...
Start-Sleep -Milliseconds 50
Write-Hacker-Fast -Text "✓ Hardware offloading configured for gaming (incl. Packet Coalescing)" -Color Green
Start-Sleep -Milliseconds 50
...
Start-Sleep -Milliseconds 50 
Write-Hacker-Fast -Text "✓ Unnecessary virtual/WAN network adapters disabled" -Color Green 
if ($qosPolicyCreated) {
    Start-Sleep -Milliseconds 50
    Write-Hacker-Fast -Text "✓ Fortnite QoS Policy for traffic prioritization created" -Color Green
}
Write-Host ""
...
if ($restart -eq "y" -or $restart -eq "Y" -or $restart -eq "yes") {
    ...
    # --- NEU: Rufen Sie die Graceful Exit-Funktion auf ---
    Graceful-Exit
}